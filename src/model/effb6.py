...
class DecoderBN(nn.Module):
    def __init__(self, num_classes, refine, num_features=256, bottleneck_features=200):
        super(DecoderBN, self).__init__()
        n_features = int(num_features)

        self.conv2 = nn.Conv2d(bottleneck_features, n_features, kernel_size=1, stride=1, padding=1)

        self.up1 = UpSampleBN(skip_input=400, output_features=n_features // 2)
        self.up2 = UpSampleBN(skip_input=200, output_features=n_features // 4)
        self.up3 = UpSampleBN(skip_input=104, output_features=n_features // 8)
